...
use jortestkit::prelude::{ProgressBarMode,parse_progress_bar_mode_from_str};
use crate::Seed;
...
pub struct QuickStartCommandArgs {
    ...
    #[structopt(long = "root-dir", default_value = "./vit_backend_data")]
...
impl QuickStartCommandArgs {
    pub fn exec(self) -> Result<()> {
        ...
        if let Some(mapping) = self.initials_mapping {
            ...
            let initials: Vec<u64> = serde_json::from_str(&content).expect("JSON was not well-formatted");
            quick_setup.initials(initials);
        } else if let Some(initials_count) = self.initials {
            quick_setup.initials_count(initials_count);
        } 

        quick_setup.vote_start_epoch(self.vote_start_epoch);
        quick_setup.tally_start_epoch(self.tally_start_epoch);
        quick_setup.tally_end_epoch(self.tally_end_epoch);
        quick_setup.slot_duration(self.slot_duration);
        quick_setup.slots_in_epoch(self.slots_in_epoch);
        quick_setup.proposals_count(self.proposals);
        quick_setup.voting_power(self.voting_power);
