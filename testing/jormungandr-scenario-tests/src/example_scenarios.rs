...
#[named]
pub fn scenario_1(context: Context) -> Result<ScenarioResult> {
...
#[named]
pub fn scenario_2(context: Context) -> Result<ScenarioResult> {
    let name = function_name!();
    let scenario_settings = prepare_scenario! {
        name,
        &mut context,
    ...
    let mut passive1 = controller.spawn_node(
        "Passive1",
        LeadershipMode::Passive,
        PersistenceMode::InMemory,
    )?;
    let mut passive2 = controller.spawn_node(
        "Passive2",
        LeadershipMode::Passive,
        PersistenceMode::InMemory,
    )?;
    let mut passive3 = controller.spawn_node(
        "Passive3",
        LeadershipMode::Passive,
        PersistenceMode::InMemory,
    )?;
