...
use jormungandr_automation::{
    jcli::JCli,
    jormungandr::{Block0ConfigurationBuilder, JormungandrBootstrapper, SecretModelFactory},
    testing::time::{get_current_date, wait_for_epoch},
};
use jormungandr_lib::{
    crypto::hash::Hash,
    interfaces::{BlockContentMaxSize, BlockDate, ConfigParam, ConfigParams, ConsensusLeaderId},
};
...
fn basic_change_config_test() {
    ...
    let alice_leader_key = startup::create_new_leader_key();
    let alice_leader_key_sk = temp_dir.child("alice_leader_key.sk");
    alice_leader_key_sk
        .write_str(&alice_leader_key.signing_key().to_bech32_str())
        .unwrap();

    let bob_leader_key = startup::create_new_leader_key();
    let bob_leader_key_sk = temp_dir.child("bob_leader_key.sk");
    bob_leader_key_sk
        .write_str(&bob_leader_key.signing_key().to_bech32_str())
        .unwrap();
    ...
    let config = Block0ConfigurationBuilder::default()
        ...
        .with_consensus_leaders_ids(vec![
            ConsensusLeaderId::from(alice_leader_key.public_key().clone()),
            ConsensusLeaderId::from(bob_leader_key.public_key().clone()),
        ])
        .build();
    ...
    let jormungandr = JormungandrBootstrapper::default()
        ...
        .with_secret(SecretModelFactory::bft(alice_leader_key.signing_key()))
        .start(temp_dir)
        .unwrap();
    ...
    let tx = jcli
        ...
        .add_auth(alice_leader_key_sk.path())
        .to_message();
    ...
    let tx = jcli
        .transaction_builder(
            Hash::from_str(&jormungandr.rest().settings().unwrap().block0_hash).unwrap(),
        )
        .new_transaction()
        .add_account(&alice.address().to_string(), &Value::zero().into())
    ...
    let update_vote_cert = jcli.certificate().new_update_vote(
        &proposal_id.to_string(),
        &bob_leader_key.public_key().to_bech32_str(),
    );
    let tx = jcli
        ...
        .add_auth(bob_leader_key_sk.path())
        .to_message();
