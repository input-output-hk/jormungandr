...
use crate::jormungandr::explorer::data::blocks_by_chain_length::{
...
impl ExplorerVerifier {
    pub fn assert_block_by_chain_length(
        block: Block,
        explorer_block: &BlocksByChainLengthBlocksByChainLength,
        ...
        match &explorer_block.leader {
            Some(leader) => match leader {
                BlocksByChainLengthBlocksByChainLengthLeader::Pool(leader) => assert_eq!(
                    leader.id,
                    block.header().get_stakepool_id().unwrap().to_string()
                ),
                BlocksByChainLengthBlocksByChainLengthLeader::BftLeader(leader) => assert_eq!(
                        ...
                        match &explorer_transaction.certificate {
                            None => {
                                if let Fragment::Transaction(fragment_transaction) = fragment {
                                    Self::assert_block_by_chain_length_transaction_param(
                                ...
                                match certificate{
                                    ...
                                    ::StakeDelegation(
                                            ...
                                            if let Fragment::StakeDelegation(fragment_cert) = fragment {
                                                ...
                                                .unwrap();
                                                Self::assert_block_by_chain_length_stake_delegation(fragment_cert, explorer_cert)
                                                    .unwrap();
                                            } else {
                                                return Err(VerifierError::InvalidCertificate {
                                        ...
                                        BlocksByChainLengthBlocksByChainLengthTransactionsEdgesNodeCertificate::OwnerStakeDelegation(
                                            explorer_cert,
                                        ) => {
                                            if let Fragment::OwnerStakeDelegation(fragment_cert) = fragment {
                                                ...
                                                .unwrap();
                                               Self::assert_block_by_chain_length_owner_delegation(fragment_cert, explorer_cert)
                                                    .unwrap();
                                            } else {
                                                return Err(VerifierError::InvalidCertificate {
                                        ...
                                        BlocksByChainLengthBlocksByChainLengthTransactionsEdgesNodeCertificate::PoolRegistration(
                                            explorer_cert,
                                        ) => {
                                            if let Fragment::PoolRegistration(fragment_cert) = fragment {
                                                ...
                                                .unwrap();
                                              Self::assert_block_by_chain_length_pool_registration(fragment_cert, explorer_cert);
                                            } else {
                                               return Err(VerifierError::InvalidCertificate {
                                        ...
                                        BlocksByChainLengthBlocksByChainLengthTransactionsEdgesNodeCertificate::PoolRetirement(
                                            explorer_cert,
                                        ) => {
                                            if let Fragment::PoolRetirement(fragment_cert) = fragment {
                                                ...
                                                .unwrap();
                                                Self::assert_block_by_chain_length_pool_retirement(fragment_cert, explorer_cert);
                                            } else {
                                               return Err(VerifierError::InvalidCertificate {
                                        ...
                                        BlocksByChainLengthBlocksByChainLengthTransactionsEdgesNodeCertificate::PoolUpdate(explorer_cert) => {
                                            if let Fragment::PoolUpdate(fragment_cert) = fragment {
                                                ...
                                                .unwrap();
                                               Self::assert_block_by_chain_length_pool_update(fragment_cert, explorer_cert);
                                            } else {
                                               return Err(VerifierError::InvalidCertificate {
                                        ...
                                        BlocksByChainLengthBlocksByChainLengthTransactionsEdgesNodeCertificate::VotePlan(explorer_cert) => {
                                            if let Fragment::VotePlan(fragment_cert) = fragment {
                                                ...
                                                .unwrap();
                                                Self::assert_block_by_chain_length_vote_plan(fragment_cert, explorer_cert);
                                            } else {
                                               return Err(VerifierError::InvalidCertificate {
                                        ...
                                        BlocksByChainLengthBlocksByChainLengthTransactionsEdgesNodeCertificate::VoteCast(explorer_cert) => {
                                            if let Fragment::VoteCast(fragment_cert) = fragment {
                                                ...
                                                .unwrap();
                                               Self::assert_block_by_chain_length_vote_cast(fragment_cert, explorer_cert);
                                            } else {
                                               return Err(VerifierError::InvalidCertificate {
                                        ...
                                        BlocksByChainLengthBlocksByChainLengthTransactionsEdgesNodeCertificate::VoteTally(explorer_cert) => {
                                            if let Fragment::VoteTally(fragment_cert) = fragment {
                                                ...
                                                .unwrap();
                                              Self::assert_block_by_chain_length_vote_tally(fragment_cert, explorer_cert);
                                            } else {
                                               return Err(VerifierError::InvalidCertificate {
                                        ...
                                        BlocksByChainLengthBlocksByChainLengthTransactionsEdgesNodeCertificate::UpdateProposal(
                                            explorer_cert,
                                        ) => {
                                            if let Fragment::UpdateProposal(fragment_cert) = fragment {
                                                ...
                                                .unwrap();
                                             Self::assert_block_by_chain_length_update_proposal(fragment_cert, explorer_cert);
                                            } else {
                                             return  Err(VerifierError::InvalidCertificate {
                                        ...
                                        BlocksByChainLengthBlocksByChainLengthTransactionsEdgesNodeCertificate::UpdateVote(explorer_cert) => {
                                            if let Fragment::UpdateVote(fragment_cert) = fragment {
                                                ...
                                                .unwrap();
                                            Self::assert_block_by_chain_length_update_vote(fragment_cert, explorer_cert);
                                            } else {
                                              return Err(VerifierError::InvalidCertificate {
                                        ...
                                        BlocksByChainLengthBlocksByChainLengthTransactionsEdgesNodeCertificate::MintToken(_) => {
                                           return Err(VerifierError::InvalidCertificate {
                                                received: "MintToken can be only in block0".to_string(),
                                            });
                                        }
                                        BlocksByChainLengthBlocksByChainLengthTransactionsEdgesNodeCertificate::EvmMapping(_) => {
    ...
    fn assert_block_by_chain_length_transaction_param<P>(
        fragment_transaction: &Transaction<P>,
        explorer_transaction: &BlocksByChainLengthBlocksByChainLengthTransactionsEdgesNode,
    ) -> Result<(), VerifierError> {
        assert_eq!(
    ...
    fn assert_block_by_chain_length_pool_registration(
        fragment_cert: &Transaction<PoolRegistration>,
        explorer_cert: &BlocksByChainLengthBlocksByChainLengthTransactionsEdgesNodeCertificateOnPoolRegistration,
    ) {
    ...
    fn assert_block_by_chain_length_stake_delegation(
        fragment_cert: &Transaction<StakeDelegation>,
        explorer_cert: &BlocksByChainLengthBlocksByChainLengthTransactionsEdgesNodeCertificateOnStakeDelegation,
    ) -> Result<(), VerifierError> {
    ...
    fn assert_block_by_chain_length_owner_delegation(
        fragment_cert: &Transaction<OwnerStakeDelegation>,
        explorer_cert: &BlocksByChainLengthBlocksByChainLengthTransactionsEdgesNodeCertificateOnOwnerStakeDelegation,
    ) -> Result<(), VerifierError> {
    ...
    fn assert_block_by_chain_length_pool_retirement(
        fragment_cert: &Transaction<PoolRetirement>,
        explorer_cert: &BlocksByChainLengthBlocksByChainLengthTransactionsEdgesNodeCertificateOnPoolRetirement,
    ) {
    ...
    fn assert_block_by_chain_length_pool_update(
        fragment_cert: &Transaction<PoolUpdate>,
        explorer_cert: &BlocksByChainLengthBlocksByChainLengthTransactionsEdgesNodeCertificateOnPoolUpdate,
    ) {
    ...
    fn assert_block_by_chain_length_vote_plan(
        fragment_cert: &Transaction<VotePlan>,
        explorer_cert: &BlocksByChainLengthBlocksByChainLengthTransactionsEdgesNodeCertificateOnVotePlan,
    ) {
    ...
    fn assert_block_by_chain_length_vote_cast(
        fragment_cert: &Transaction<VoteCast>,
        explorer_cert: &BlocksByChainLengthBlocksByChainLengthTransactionsEdgesNodeCertificateOnVoteCast,
    ) {
    ...
    fn assert_block_by_chain_length_vote_tally(
        fragment_cert: &Transaction<VoteTally>,
        explorer_cert: &BlocksByChainLengthBlocksByChainLengthTransactionsEdgesNodeCertificateOnVoteTally,
    ) {
    ...
    fn assert_block_by_chain_length_update_proposal(
        fragment_cert: &Transaction<UpdateProposal>,
        explorer_cert: &BlocksByChainLengthBlocksByChainLengthTransactionsEdgesNodeCertificateOnUpdateProposal,
    ) {
    ...
    fn assert_block_by_chain_length_update_vote(
        fragment_cert: &Transaction<UpdateVote>,
        explorer_cert: &BlocksByChainLengthBlocksByChainLengthTransactionsEdgesNodeCertificateOnUpdateVote,
    ) {
