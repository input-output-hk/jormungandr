...
use crate::jormungandr::explorer::data::all_blocks::{PayloadType as expPayloadType, *};
...
impl ExplorerVerifier {
    pub fn assert_all_blocks(
        block: Block,
        explorer_block: &AllBlocksTipBlocksEdgesNode,
        ...
        match &explorer_block.leader {
            Some(leader) => match leader {
                AllBlocksTipBlocksEdgesNodeLeader::Pool(leader) => assert_eq!(
                    leader.id,
                    block.header().get_stakepool_id().unwrap().to_string()
                ),
                AllBlocksTipBlocksEdgesNodeLeader::BftLeader(leader) => assert_eq!(
        ...
        assert_eq!(
            explorer_block.transactions.total_count + 1,
                        ...
                        match &explorer_transaction.certificate {
                            None => {
                                if let Fragment::Transaction(fragment_transaction) = fragment {
                                    Self::assert_all_blocks_transaction_param(
                            ...
                            Some(certificate) => {
                                match certificate{
                                        AllBlocksTipBlocksEdgesNodeTransactionsEdgesNodeCertificate::StakeDelegation(
                                            explorer_cert,
                                        ) => {
                                            if let Fragment::StakeDelegation(fragment_cert) = fragment {
                                                ...
                                                .unwrap();
                                                Self::assert_all_blocks_stake_delegation(fragment_cert, explorer_cert)
                                                    .unwrap();
                                            } else {
                                                return Err(VerifierError::InvalidCertificate {
                                        ...
                                        AllBlocksTipBlocksEdgesNodeTransactionsEdgesNodeCertificate::OwnerStakeDelegation(
                                            explorer_cert,
                                        ) => {
                                            if let Fragment::OwnerStakeDelegation(fragment_cert) = fragment {
                                                ...
                                                .unwrap();
                                               Self::assert_all_blocks_owner_delegation(fragment_cert, explorer_cert)
                                                    .unwrap();
                                            } else {
                                                return Err(VerifierError::InvalidCertificate {
                                        ...
                                        AllBlocksTipBlocksEdgesNodeTransactionsEdgesNodeCertificate::PoolRegistration(
                                            explorer_cert,
                                        ) => {
                                            if let Fragment::PoolRegistration(fragment_cert) = fragment {
                                                ...
                                                .unwrap();
                                              Self::assert_all_blocks_pool_registration(fragment_cert, explorer_cert);
                                            } else {
                                               return Err(VerifierError::InvalidCertificate {
                                        ...
                                        AllBlocksTipBlocksEdgesNodeTransactionsEdgesNodeCertificate::PoolRetirement(
                                            explorer_cert,
                                        ) => {
                                            if let Fragment::PoolRetirement(fragment_cert) = fragment {
                                                ...
                                                .unwrap();
                                                Self::assert_all_blocks_pool_retirement(fragment_cert, explorer_cert);
                                            } else {
                                               return Err(VerifierError::InvalidCertificate {
                                        ...
                                        AllBlocksTipBlocksEdgesNodeTransactionsEdgesNodeCertificate::PoolUpdate(explorer_cert) => {
                                            if let Fragment::PoolUpdate(fragment_cert) = fragment {
                                                ...
                                                .unwrap();
                                               Self::assert_all_blocks_pool_update(fragment_cert, explorer_cert);
                                            } else {
                                               return Err(VerifierError::InvalidCertificate {
                                        ...
                                        AllBlocksTipBlocksEdgesNodeTransactionsEdgesNodeCertificate::VotePlan(explorer_cert) => {
                                            if let Fragment::VotePlan(fragment_cert) = fragment {
                                                ...
                                                .unwrap();
                                                Self::assert_all_blocks_vote_plan(fragment_cert, explorer_cert);
                                            } else {
                                               return Err(VerifierError::InvalidCertificate {
                                        ...
                                        AllBlocksTipBlocksEdgesNodeTransactionsEdgesNodeCertificate::VoteCast(explorer_cert) => {
                                            if let Fragment::VoteCast(fragment_cert) = fragment {
                                                ...
                                                .unwrap();
                                               Self::assert_all_blocks_vote_cast(fragment_cert, explorer_cert);
                                            } else {
                                               return Err(VerifierError::InvalidCertificate {
                                        ...
                                        AllBlocksTipBlocksEdgesNodeTransactionsEdgesNodeCertificate::VoteTally(explorer_cert) => {
                                            if let Fragment::VoteTally(fragment_cert) = fragment {
                                                ...
                                                .unwrap();
                                              Self::assert_all_blocks_vote_tally(fragment_cert, explorer_cert);
                                            } else {
                                               return Err(VerifierError::InvalidCertificate {
                                        ...
                                        AllBlocksTipBlocksEdgesNodeTransactionsEdgesNodeCertificate::UpdateProposal(
                                            explorer_cert,
                                        ) => {
                                            if let Fragment::UpdateProposal(fragment_cert) = fragment {
                                                ...
                                                .unwrap();
                                             Self::assert_all_blocks_update_proposal(fragment_cert, explorer_cert);
                                            } else {
                                             return  Err(VerifierError::InvalidCertificate {
                                        ...
                                        AllBlocksTipBlocksEdgesNodeTransactionsEdgesNodeCertificate::UpdateVote(explorer_cert) => {
                                            if let Fragment::UpdateVote(fragment_cert) = fragment {
                                                ...
                                                .unwrap();
                                            Self::assert_all_blocks_update_vote(fragment_cert, explorer_cert);
                                            } else {
                                              return Err(VerifierError::InvalidCertificate {
                                        ...
                                        AllBlocksTipBlocksEdgesNodeTransactionsEdgesNodeCertificate::MintToken(_) => {
                                           return Err(VerifierError::InvalidCertificate {
                                                received: "MintToken can be only in block0".to_string(),
                                            });
                                        }
                                        AllBlocksTipBlocksEdgesNodeTransactionsEdgesNodeCertificate::EvmMapping(_) => {
    ...
    fn assert_all_blocks_transaction_param<P>(
        fragment_transaction: &Transaction<P>,
        explorer_transaction: &AllBlocksTipBlocksEdgesNodeTransactionsEdgesNode,
    ) -> Result<(), VerifierError> {
        assert_eq!(
    ...
    fn assert_all_blocks_pool_registration(
        fragment_cert: &Transaction<PoolRegistration>,
        explorer_cert: &AllBlocksTipBlocksEdgesNodeTransactionsEdgesNodeCertificateOnPoolRegistration,
    ) {
    ...
    fn assert_all_blocks_stake_delegation(
        fragment_cert: &Transaction<StakeDelegation>,
        explorer_cert: &AllBlocksTipBlocksEdgesNodeTransactionsEdgesNodeCertificateOnStakeDelegation,
    ) -> Result<(), VerifierError> {
    ...
    fn assert_all_blocks_owner_delegation(
        fragment_cert: &Transaction<OwnerStakeDelegation>,
        explorer_cert: &AllBlocksTipBlocksEdgesNodeTransactionsEdgesNodeCertificateOnOwnerStakeDelegation,
    ) -> Result<(), VerifierError> {
    ...
    fn assert_all_blocks_pool_retirement(
        fragment_cert: &Transaction<PoolRetirement>,
        explorer_cert: &AllBlocksTipBlocksEdgesNodeTransactionsEdgesNodeCertificateOnPoolRetirement,
    ) {
    ...
    fn assert_all_blocks_pool_update(
        fragment_cert: &Transaction<PoolUpdate>,
        explorer_cert: &AllBlocksTipBlocksEdgesNodeTransactionsEdgesNodeCertificateOnPoolUpdate,
    ) {
    ...
    fn assert_all_blocks_vote_plan(
        fragment_cert: &Transaction<VotePlan>,
        explorer_cert: &AllBlocksTipBlocksEdgesNodeTransactionsEdgesNodeCertificateOnVotePlan,
    ) {
    ...
    fn assert_all_blocks_vote_cast(
        fragment_cert: &Transaction<VoteCast>,
        explorer_cert: &AllBlocksTipBlocksEdgesNodeTransactionsEdgesNodeCertificateOnVoteCast,
    ) {
    ...
    fn assert_all_blocks_vote_tally(
        fragment_cert: &Transaction<VoteTally>,
        explorer_cert: &AllBlocksTipBlocksEdgesNodeTransactionsEdgesNodeCertificateOnVoteTally,
    ) {
    ...
    fn assert_all_blocks_update_proposal(
        fragment_cert: &Transaction<UpdateProposal>,
        explorer_cert: &AllBlocksTipBlocksEdgesNodeTransactionsEdgesNodeCertificateOnUpdateProposal,
    ) {
    ...
    fn assert_all_blocks_update_vote(
        fragment_cert: &Transaction<UpdateVote>,
        explorer_cert: &AllBlocksTipBlocksEdgesNodeTransactionsEdgesNodeCertificateOnUpdateVote,
    ) {
