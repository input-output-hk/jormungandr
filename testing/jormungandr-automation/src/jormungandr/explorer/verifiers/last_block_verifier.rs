...
use crate::jormungandr::explorer::{
    data::last_block::{PayloadType as expPayloadType, *},
    last_block::LastBlockTipBlockTransactionsEdgesNode,
};
...
impl ExplorerVerifier {
    pub fn assert_last_block(
        block: Block,
        explorer_block: &LastBlockTipBlock,
        ...
        match &explorer_block.leader {
            Some(leader) => match leader {
                LastBlockTipBlockLeader::Pool(leader) => assert_eq!(
                    leader.id,
                    block.header().get_stakepool_id().unwrap().to_string()
                ),
                LastBlockTipBlockLeader::BftLeader(leader) => assert_eq!(
                ...
                for edge in explorer_block.transactions.edges.as_ref().unwrap() {
                    ...
                    if fragment.hash().to_string() == explorer_transaction.id {
                        ...
                        match &explorer_transaction.certificate {
                            ...
                            Some(certificate) => {
                                match certificate{
                                        LastBlockTipBlockTransactionsEdgesNodeCertificate::StakeDelegation(
                                            explorer_cert,
                                        ) => {
                                        ...
                                        LastBlockTipBlockTransactionsEdgesNodeCertificate::OwnerStakeDelegation(
                                            explorer_cert,
                                        ) => {
                                        ...
                                        LastBlockTipBlockTransactionsEdgesNodeCertificate::PoolRegistration(
                                            explorer_cert,
                                        ) => {
                                        ...
                                        LastBlockTipBlockTransactionsEdgesNodeCertificate::PoolRetirement(
                                            explorer_cert,
                                        ) => {
                                        ...
                                        LastBlockTipBlockTransactionsEdgesNodeCertificate::PoolUpdate(explorer_cert) => {
                                        ...
                                        LastBlockTipBlockTransactionsEdgesNodeCertificate::VotePlan(explorer_cert) => {
                                        ...
                                        LastBlockTipBlockTransactionsEdgesNodeCertificate::VoteCast(explorer_cert) => {
                                        ...
                                        LastBlockTipBlockTransactionsEdgesNodeCertificate::VoteTally(explorer_cert) => {
                                        ...
                                        LastBlockTipBlockTransactionsEdgesNodeCertificate::UpdateProposal(
                                            explorer_cert,
                                        ) => {
                                        ...
                                        LastBlockTipBlockTransactionsEdgesNodeCertificate::UpdateVote(explorer_cert) => {
                                        ...
                                        LastBlockTipBlockTransactionsEdgesNodeCertificate::MintToken(_) => {
                                           return Err(VerifierError::InvalidCertificate {
                                                received: "MintToken can be only in block0".to_string(),
                                            });
                                        }
                                        LastBlockTipBlockTransactionsEdgesNodeCertificate::EvmMapping(_) => {
    ...
    fn assert_last_block_transaction_param<P>(
    ...
    fn assert_last_block_pool_registration(
        fragment_cert: &Transaction<PoolRegistration>,
        explorer_cert: &LastBlockTipBlockTransactionsEdgesNodeCertificateOnPoolRegistration,
    ) {
    ...
    fn assert_last_block_stake_delegation(
        ...
        explorer_cert: &LastBlockTipBlockTransactionsEdgesNodeCertificateOnStakeDelegation,
    ) -> Result<(), VerifierError> {
    ...
    fn assert_last_block_owner_delegation(
        fragment_cert: &Transaction<OwnerStakeDelegation>,
        explorer_cert: &LastBlockTipBlockTransactionsEdgesNodeCertificateOnOwnerStakeDelegation,
    ) -> Result<(), VerifierError> {
    ...
    fn assert_last_block_pool_retirement(
        fragment_cert: &Transaction<PoolRetirement>,
        explorer_cert: &LastBlockTipBlockTransactionsEdgesNodeCertificateOnPoolRetirement,
    ) {
    ...
    fn assert_last_block_pool_update(
        fragment_cert: &Transaction<PoolUpdate>,
        explorer_cert: &LastBlockTipBlockTransactionsEdgesNodeCertificateOnPoolUpdate,
    ) {
    ...
    fn assert_last_block_vote_plan(
        fragment_cert: &Transaction<VotePlan>,
        explorer_cert: &LastBlockTipBlockTransactionsEdgesNodeCertificateOnVotePlan,
    ) {
    ...
    fn assert_last_block_vote_cast(
        fragment_cert: &Transaction<VoteCast>,
        explorer_cert: &LastBlockTipBlockTransactionsEdgesNodeCertificateOnVoteCast,
    ) {
    ...
    fn assert_last_block_vote_tally(
        fragment_cert: &Transaction<VoteTally>,
        explorer_cert: &LastBlockTipBlockTransactionsEdgesNodeCertificateOnVoteTally,
    ) {
    ...
    fn assert_last_block_update_proposal(
        fragment_cert: &Transaction<UpdateProposal>,
        explorer_cert: &LastBlockTipBlockTransactionsEdgesNodeCertificateOnUpdateProposal,
    ) {
    ...
    fn assert_last_block_update_vote(
        fragment_cert: &Transaction<UpdateVote>,
        explorer_cert: &LastBlockTipBlockTransactionsEdgesNodeCertificateOnUpdateVote,
    ) {
