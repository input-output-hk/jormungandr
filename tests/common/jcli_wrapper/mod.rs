...
pub fn assert_rest_stats(host: &str) -> BTreeMap<String, String> {
    let output = process_utils::run_process_and_get_output(get_rest_stats_command(&host));
...
pub fn assert_rest_utxo_get(host: &str) -> Vec<Utxo> {
    let output = process_utils::run_process_and_get_output(get_rest_utxo_get_command(&host));
...
fn get_rest_utxo_get_command(host: &str) -> Command {
    let mut command = Command::new(configuration::get_jcli_app().as_os_str());
    command
        .arg("rest")
        .arg("v0")
...
pub fn assert_post_transaction(transaction_hash: &str, host: &str) -> () {
    let output = process_utils::run_process_and_get_output(get_post_transaction_command(
        &transaction_hash,
        &host,
    ));
    let single_line = output.as_single_line();
