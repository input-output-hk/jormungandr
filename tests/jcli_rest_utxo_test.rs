...
pub fn test_correct_utxos_are_read_from_node() {
    let sender_private_key = jcli_wrapper::assert_key_generate_command_default();
    println!("Sender private key generated: {}", &sender_private_key);

    let reciever_private_key = jcli_wrapper::assert_key_generate_command_default();
    println!("Reciever private key generated: {}", &reciever_private_key);

    let sender_public_key = jcli_wrapper::assert_key_to_public_command_default(&sender_private_key);
    println!("Sender public key generated: {}", &sender_public_key);

    let reciever_public_key =
        jcli_wrapper::assert_key_to_public_command_default(&reciever_private_key);
    println!("Reciever public key generated: {}", &reciever_public_key);

    let sender_address = jcli_wrapper::assert_address_single_command_default(&sender_public_key);
    println!("Sender address generated: {}", &sender_address);

    let reciever_address =
        jcli_wrapper::assert_address_single_command_default(&reciever_public_key);
    ...
    let content = jcli_wrapper::assert_rest_utxo_get_command_default();
