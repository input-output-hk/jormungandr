...
mod tests {
    ...
    #[test]
    fn test_vector_1() {
        let s = "DdzFFzCqrhsrcTVhLygT24QwTnNqQqQ8mZrq5jykUzMveU26sxaH529kMpo7VhPrt5pwW3dXeB2k3EEvKcNBRmzCfcQ7dTkyGzTs658C";
        let bytes = [0x6a, 0x50, 0x96, 0x89, 0xc6, 0x53, 0x17, 0x58, 0x65, 0x98, 0x5a, 0xd1, 0xe0, 0xeb, 0x5f, 0xf9, 0xad, 0xa6, 0x99, 0x7a, 0xa4, 0x03, 0xe6, 0x48, 0x61, 0x4b, 0x3b, 0x78, 0xfc, 0xba, 0x9c, 0x27, 0x30, 0x82, 0x28, 0xd9, 0x87, 0x2a, 0xf8, 0xb6, 0x5b, 0x98, 0x7f, 0xf2, 0x3e, 0x1a, 0x20, 0xcd, 0x90, 0xd8, 0x34, 0x6c, 0x31, 0xf0, 0xed, 0xb8, 0x99, 0x89, 0x52, 0xdc, 0x67, 0x66, 0x55, 0x80];
        let addr = super::Addr::from_str(&s).unwrap();
        match XPub::from_slice(&bytes) {
            Ok(xpub) => assert!(addr.identical_with_pubkey(&xpub)),
            _ => assert!(false, "invalid xpub"),
        }
    }

    #[test]
    fn test_vector_2() {
        let s = "DdzFFzCqrht4it4GYgBp4J39FNnKBsPFejSppARXHCf2gGiTJcwXzpRvgDmxPvKQ8aZZmVqcLUz5L66a8Ja46pfKVtFRaKyn9eKdvpaC";
        let bytes = [0xff, 0x7b, 0xf1, 0x29, 0x9d, 0xf3, 0xd7, 0x17, 0x98, 0xae, 0xfd, 0xc4, 0xae, 0xa7, 0xdb, 0x2f, 0x8d, 0xb7, 0x60, 0x46, 0x56, 0x94, 0x41, 0xea, 0xe5, 0x8b, 0x72, 0x23, 0xb6, 0x8b, 0x44, 0x04, 0x82, 0x15, 0xcb, 0xac, 0x94, 0xbc, 0xb7, 0xf2, 0xcf, 0x33, 0x6c, 0x6c, 0x18, 0xbc, 0x3e, 0x71, 0x3f, 0xfd, 0x82, 0x67, 0x59, 0x4f, 0xf6, 0x34, 0x93, 0x32, 0xce, 0x4f, 0x98, 0x04, 0xa7, 0xff];
        let addr = super::Addr::from_str(&s).unwrap();
        match XPub::from_slice(&bytes) {
            Ok(xpub) => assert!(addr.identical_with_pubkey(&xpub)),
            _ => assert!(false, "invalid xpub"),
    ...
    #[test]
    fn test_vector_3() {
        let s = "DdzFFzCqrhsvNQtyViTvEdGxfdc5T1E5RorzFWjYodqjhFDy8fQxfDPccmTc4ePbvkiwvRkR8dtqQ1SHpH53fDSoxD17fo9f6WkRjjAA";
        let bytes = [0x5c, 0x36, 0x51, 0xe0, 0xeb, 0x9d, 0x6d, 0xc9, 0x64, 0x07, 0x13, 0x7c, 0xcc, 0x1f, 0x37, 0x7a, 0x87, 0x94, 0x61, 0x77, 0xa5, 0x2c, 0xa3, 0x77, 0x2c, 0x6b, 0x4b, 0xeb, 0x72, 0x39, 0x50, 0xdc, 0x50, 0x22, 0x46, 0x68, 0x21, 0x8b, 0x8b, 0x36, 0x62, 0x02, 0xfe, 0x5b, 0x7d, 0x55, 0x6f, 0x50, 0x1c, 0x5c, 0x4e, 0x2d, 0x58, 0xe0, 0x54, 0x67, 0xe1, 0xab, 0xc0, 0x44, 0xc6, 0xc1, 0xbf, 0x8e];
        let addr = super::Addr::from_str(&s).unwrap();
        match XPub::from_slice(&bytes) {
            Ok(xpub) => assert!(addr.identical_with_pubkey(&xpub)),
            _ => assert!(false, "invalid xpub"),
    ...
    #[test]
    fn test_vector_4() {
        let s = "DdzFFzCqrhsn7ZAhKy8mxkzW6G3wryM7K6bH38VAjE2FesJMxia3UviivMvGz146TP1FpDharxTE6nUgCCnZx2fmtKpmxAosg9Tf5b8y";
        let bytes = [0xcd, 0x84, 0x2e, 0x01, 0x0d, 0x81, 0xa6, 0xbe, 0x1e, 0x16, 0x9f, 0xd6, 0x35, 0x21, 0xdb, 0xb9, 0x5f, 0x42, 0x41, 0xfc, 0x82, 0x3f, 0x45, 0xb1, 0xcf, 0x1a, 0x1c, 0xb4, 0xc5, 0x89, 0x57, 0x27, 0x1d, 0x4d, 0x14, 0x2a, 0x22, 0x94, 0xea, 0x5f, 0xa3, 0x16, 0xa4, 0xad, 0xbf, 0xcd, 0x59, 0x7a, 0x7c, 0x89, 0x6a, 0x52, 0xa9, 0xa3, 0xa9, 0xce, 0x49, 0x64, 0x4a, 0x10, 0x2d, 0x00, 0x71, 0x99];
        let addr = super::Addr::from_str(&s).unwrap();
        match XPub::from_slice(&bytes) {
            Ok(xpub) => assert!(addr.identical_with_pubkey(&xpub)),
            _ => assert!(false, "invalid xpub"),
    ...
    #[test]
    fn test_vector_5() {
        let s = "DdzFFzCqrhssTCJf4sv664bdQURovAwzx1hNKkMkNLwMNyaxZFuPSDdZTTRMcoDyXHuCiZhbD4umvMJcWGkvFMMzBoBUW5UBdBbDqXGX";
        let bytes = [0x5a, 0xac, 0x2d, 0xd0, 0xa8, 0xdc, 0x5d, 0x61, 0x0a, 0x4b, 0x6f, 0xdf, 0x3f, 0x5e, 0xf1, 0xb6, 0x4a, 0xcb, 0x76, 0xb1, 0xe8, 0x1f, 0x6a, 0x35, 0x70, 0x31, 0xfa, 0x19, 0xd5, 0xe6, 0x56, 0x9d, 0xcc, 0x37, 0xb7, 0xae, 0x6f, 0x39, 0x15, 0x82, 0xfb, 0x05, 0x4b, 0x72, 0xba, 0xda, 0x90, 0xab, 0x14, 0x6c, 0xdd, 0x01, 0x42, 0x0e, 0x4b, 0x40, 0x18, 0xf1, 0xa0, 0x55, 0x29, 0x82, 0xd2, 0x31];
        let addr = super::Addr::from_str(&s).unwrap();
        match XPub::from_slice(&bytes) {
            Ok(xpub) => assert!(addr.identical_with_pubkey(&xpub)),
            _ => assert!(false, "invalid xpub"),
    ...
    #[test]
    fn test_vector_6() {
        let s = "DdzFFzCqrhsfi5fFjJUHYPSnfTYrnMohzh3PrrtrVQgwua33HWPKUdTJXo3o77pSGCmDNrjYaAiZmJddaPW9iHyUDatvU2WhX7MgnNMy";
        let bytes = [0x2a, 0x6a, 0xd1, 0x51, 0x09, 0x96, 0xff, 0x2d, 0x10, 0x89, 0xcb, 0x8e, 0xd5, 0xf5, 0xc0, 0x61, 0xf6, 0xad, 0x0a, 0xfb, 0xb5, 0x3d, 0x95, 0x40, 0xa0, 0xfc, 0x89, 0xef, 0xc0, 0xa2, 0x63, 0xb9, 0x6d, 0xac, 0x00, 0xbd, 0x0d, 0x7b, 0xda, 0x7d, 0x16, 0x3a, 0x08, 0xdb, 0x20, 0xba, 0x64, 0xb6, 0x33, 0x4d, 0xca, 0x34, 0xea, 0xc8, 0x2c, 0xf7, 0xb4, 0x91, 0xc3, 0x5f, 0x5c, 0xae, 0xc7, 0xb0];
        let addr = super::Addr::from_str(&s).unwrap();
        match XPub::from_slice(&bytes) {
            Ok(xpub) => assert!(addr.identical_with_pubkey(&xpub)),
            _ => assert!(false, "invalid xpub"),
    ...
    #[test]
    fn test_vector_9() {
        let s = "DdzFFzCqrhssTywqjv3dw3EakpEydWQcc3phQzR3YF9NPgQN9Ftkx68FfLLnpJ4vhWo9mAjx5EcpM1wNvorSySrpARZGfk5QugHkVs58";
        let bytes = [0x16, 0xf7, 0xd2, 0x55, 0x32, 0x6d, 0x77, 0x6e, 0xc1, 0xb5, 0xed, 0xd2, 0x5f, 0x75, 0xd3, 0xe3, 0xeb, 0xe0, 0xb9, 0xd4, 0x9c, 0xdd, 0xb2, 0x46, 0xd8, 0x0c, 0xf4, 0x1b, 0x25, 0x24, 0x64, 0xb6, 0x24, 0x50, 0xa2, 0x4e, 0xf5, 0x98, 0x7b, 0x4b, 0xd6, 0x5e, 0x0d, 0x25, 0x23, 0x43, 0xab, 0xa8, 0xef, 0x77, 0x93, 0x34, 0x79, 0xde, 0xa8, 0xdd, 0xe2, 0x9e, 0xec, 0x56, 0xcc, 0x6a, 0xc0, 0x69];
        let addr = super::Addr::from_str(&s).unwrap();
        match XPub::from_slice(&bytes) {
            Ok(xpub) => assert!(addr.identical_with_pubkey(&xpub)),
            _ => assert!(false, "invalid xpub"),
    ...
    #[test]
    fn test_vector_10() {
        let s = "DdzFFzCqrhsqTG4t3uq5UBqFrxhxGVM6bvF4q1QcZXqUpizFddEEip7dx5rbife2s9o2fRU3hVKhRp4higog7As8z42s4AMw6Pcu8vL4";
        let bytes = [0x97, 0xb8, 0x6c, 0x69, 0xd1, 0x2a, 0xf1, 0x64, 0xdc, 0x87, 0xf2, 0x71, 0x26, 0x8f, 0x33, 0xbc, 0x4d, 0xee, 0xb0, 0xdf, 0xd3, 0x73, 0xc3, 0xfd, 0x3b, 0xac, 0xd4, 0x47, 0x53, 0xa3, 0x1d, 0xe7, 0x8f, 0x10, 0xe5, 0x55, 0x03, 0x7c, 0xd4, 0x00, 0x43, 0x6c, 0xcf, 0xd5, 0x38, 0x0d, 0xbb, 0xcd, 0x4d, 0x7c, 0x28, 0x0a, 0xef, 0x9e, 0xc7, 0x57, 0x4a, 0xe0, 0xac, 0xac, 0x0c, 0xf7, 0x9e, 0x89];
        let addr = super::Addr::from_str(&s).unwrap();
        match XPub::from_slice(&bytes) {
            Ok(xpub) => assert!(addr.identical_with_pubkey(&xpub)),
            _ => assert!(false, "invalid xpub"),
