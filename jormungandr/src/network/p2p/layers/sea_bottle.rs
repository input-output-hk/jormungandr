...
pub fn stake_pools_sorted_by<F>(block_ref: Arc<blockchain::Ref>, comp: F) -> impl IntoIterator<Item = PoolId>
    where
...
/// ```
pub fn cmp_pool_stake_information_by_stake_total(pool_a: &PoolStakeInformation, pool_b: &PoolStakeInformation) -> Ordering {
...
pub mod test {
    ...
    #[test]
    fn foo() {
        let mut v : Vec<&PoolStakeInformation> = Vec::new();
        v.sort_by(cmp_pool_stake_information_by_stake_total);
    }
}