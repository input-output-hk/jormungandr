use crate::{
    ...
    block::{BlockDate,ChainLength,BlockBuilder,Block, Header},
    key::Hash
};
use chain_crypto::vrf_evaluate_and_prove;
use chain_core::property::{Header as h,ChainLength as ch};
...
impl GenesisPraosBlockBuilder {
    ...
    pub fn build(&self, stake_pool: &StakePool, time_era: &TimeEra) -> Block {
        ...
        block_builder.chain_length(self.chain_length.unwrap().next());
