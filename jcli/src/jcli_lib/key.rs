...
use chain_crypto::{
    ...
    Ed25519Extended, EllipticCurve2hashDh, SecretKey, SigningAlgorithm, SumEd25519_12,
...
impl Generate {
    fn exec(self) -> Result<(), Error> {
        let priv_key_bech32 = match self.key_type {
            ...
            GenPrivKeyType::EllipticCurve2hashDh => {
                gen_priv_key::<EllipticCurve2hashDh>(self.seed)?
            }
        };
...
impl ToPublic {
    fn exec(self) -> Result<(), Error> {
        ...
        let pub_key_bech32 = match hrp.as_ref() {
            ...
            EllipticCurve2hashDh::SECRET_BECH32_HRP => gen_pub_key::<EllipticCurve2hashDh>(&data),
...
impl FromBytes {
    fn exec(self) -> Result<(), Error> {
        ...
        let priv_key_bech32 = match self.key_type {
            ...
            GenPrivKeyType::EllipticCurve2hashDh => {
                bytes_to_priv_key::<EllipticCurve2hashDh>(&bytes)?
            }
        };
        let mut output = self.output_file.open()?;
        writeln!(output, "{}", priv_key_bech32)?;
