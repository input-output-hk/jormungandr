...
use crate::db::indexing::ExplorerTransaction;
    ...
    pub fn validate(self) -> FieldResult<ValidatedPaginationArguments<I>> {
        let first = self
            ...
            .map(|signed| -> FieldResult<u32> {
                if signed < 0 {
                    Err(Error::ArgumentError("first argument should be positive".to_owned()).into())
                } else {
        ...
        let last = self
            ...
            .map(|signed| -> FieldResult<u32> {
                if signed < 0 {
                    Err(Error::ArgumentError("last argument should be positive".to_owned()).into())
                } else {
                    Ok(u32::try_from(signed).unwrap())
                }
            })
